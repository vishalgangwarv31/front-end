...
const UpdateOrder: React.FC = () => {
    ...
    const fetchOrder = async () => {
      try {
        ...
        setFormData({
          ...
          govtAppNumber: order.govtAppNumber?.toString() || '',
          dateOfFilling: order.dateOfFilling || '',
          lawyerRefrenceNumber: order.lawyerRefrenceNumber?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await axios.put(`http://localhost:3000/api/admin/update-order/${id}`, data, {
        headers: {
          'Authorization': `Bearer ${token}`,
